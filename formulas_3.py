...
def radius_input(*args):
    try:
        ...
        try:
            ...
            try:
                if '/' in output:
                    fraction_mod = output.split('/')
                    numerator = float(fraction_mod[0])
                    denominator = float(fraction_mod[1])
                    radius = numerator / denominator
                else:
                    box.showinfo("Invalid Input", "Sorry, you forgot the input or caused an unknown error!")
            except Exception:
                pass
    try:
        A = pi * (radius ** 2)
        ...
        if A < A // 1 + 0.499995:
            ...
            box.showinfo("Easter Egg", "No longer any easter egg :(")
    ...
    try:
        ...
        try:
            ...
            try:
                if '/' in r:
                    r_split = r.split("/")
                    numerator = float(r_split[0])
                    denominator = float(r_split[1])
    ...
    try:
        ...
        try:
            ...
            try:
                if '/' in t:
                    t_split = t.split("/")
                    first_num = float(t_split[0])
                    second_num = float(t_split[1])
